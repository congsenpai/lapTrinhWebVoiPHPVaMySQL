        ...
        <div class="card w-100">
            <div class="card-body p-4">
                <div class="d-flex align-items-center justify-content-between mb-4" style="padding-bottom: 12px">
                    ...
                    <a href="{{route('category.create')}}" class="btn btn-primary m-1" style="margin-left:5px">Tạo mới</a>
                </div>
                        ...
                        <tbody>
                            @foreach($categories as $category)
                                <tr>
                                    ...
                                    <td class="border-bottom-0 text-center d-flex justify-content-center" style="align-items: center">
                                        <a href="{{route('category.edit', $category)}}" class="btn btn-info btn-sm" style="line-height:2; height:36px">Sửa</a>
                                        <form action="{{route('category.destroy', $category)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            <button onclick="return confirm('Do you really want to delete this items?')"
                                            type="submit" class="btn btn-danger btn-sm " style="margin: 0 5px 0 5px; height:36px ">Xóa</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
