...
(function ($) {
    ...
    let $clone_to_mobile_sidebar = $(".clone-to-mobile-sidebar"),
        ...
        BIOLIFE_MOBILE_MENU = {
            ...
            clone_menus: function (obj) {
                ...
                obj.each(function () {
                    ...
                    if (!$("#" + $this_menu_clone_id).length) {
                        ...
                        $thisClone
                            .find(".menu-item")
                            .addClass("clone-menu-item");
                        $thisClone.find("[id]").each(function () {
                            $__this.attr(
                                "id",
                                BIOLIFE_MOBILE_MENU.add_string_prefix(
                                    this_id,
                                    "biolife-clone-"
                                )
                            );
                        ...
                        $thisClone
                            .find(".biolife-menu")
                            .addClass("biolife-menu-clone");
                        let $thisMainPanel = $(
                            "#biolife-clone-wrap .biolife-panels #biolife-main-panel ul"
                        );
                ...
                $document.on("click", ".biolife-next-panel", function (e) {
                    ...
                    if ($(target_id).length) {
                        ...
                        $(target_id)
                            .addClass("biolife-panel-opened")
                            .removeClass("biolife-hidden")
                            .attr("data-parent-panel", thisPanel.attr("id"));
                        /*Insert current panel title*/
                        let item_title = thisItem
                                .find(".menu-name")
                                .attr("data-title"),
                            firstItemTitle = "",
                            $biolife_panels = $(".biolife-panels-actions-wrap"),
                            current_panel_title = $biolife_panels.find(
                                ".biolife-current-panel-title"
                            );
                        ...
                        if (typeof item_title !== "undefined") {
                            if (current_panel_title.length) {
                                ...
                                $biolife_panels.prepend(
                                    '<span class="biolife-current-panel-title">' +
                                        item_title +
                                        "</span>"
                                );
                            }
                ...
                $document.on("click", ".biolife-prev-panel", function (e) {
                    ...
                    $(cur_panel_id)
                        .removeClass("biolife-panel-opened")
                        .addClass("biolife-hidden");
                    $(target_id)
                        .addClass("biolife-panel-opened")
                        .removeClass("biolife-sub-opened");

                    /*Set new back button*/
                    let new_parent_panel_id =
                        $(target_id).attr("data-parent-panel");
                    if (typeof new_parent_panel_id == "undefined") {
                        let root_title = $(".biolife-panels-actions-wrap").data(
                            "root_title"
                        );
                        ...
                        let item_title = $("#" + new_parent_panel_id)
                            .find(
                                '.biolife-next-panel[data-target="' +
                                    target_id +
                                    '"]'
                            )
                            .closest(".menu-item")
                        ...
                        if (typeof item_title !== "undefined") {
                            if (!current_panel_title.length) {
                                $(".biolife-panels-actions-wrap").prepend(
                                    '<span class="biolife-current-panel-title">' +
                                        item_title +
                                        "</span>"
                                );
                ...
                setTimeout(function () {
                    $("#biolife-clone-wrap .biolife-panels")
                        .find(".biolife-carousel")
                        .biolife_init_carousel();
                }, 1000);
            },
            ...
            insert_children: function (obj, i) {
                ...
                if (children.length) {
                    children.each(function () {
                        ...
                        BIOLIFE_MOBILE_MENU.insert_children(
                            thisChildItem,
                            index
                        );
                        ...
                        thisChildItem.prepend(
                            '<a class="biolife-next-panel" href="#' +
                                next_nav_target +
                                '" data-target="#' +
                                next_nav_target +
                                '"></a>'
                        );
                        let submenu_html = $("<div>")
                            .append(
                                thisChildItem
                                    .find(">.sub-menu,>.wrap-megamenu")
                                    .clone()
                            )
                            .html();
                        thisChildItem
                            .find("> .sub-menu,> .wrap-megamenu")
                            .remove();
                        $("#biolife-clone-wrap .biolife-panels").append(
                            '<div id="' +
                                next_nav_target +
                                '" class="biolife-panel biolife-sub-panel biolife-hidden">' +
                                submenu_html +
                                "</div>"
                        );
                    });
            ...
            wrap_container: function () {
                ...
                mb_menu +=
                    '<div class="biolife-panels-actions-wrap" data-root_title="Main Panel">';
                mb_menu +=
                    '<span class="biolife-current-panel-title">Main Panel</span>';
                mb_menu +=
                    '<a class="biolife-close-btn biolife-close-panels" href="#" data-object="open-mobile-menu">&times;</a>';
                mb_menu += "</div>";
    ...
    $.fn.biolife_init_carousel = function () {
        if (typeof $.fn.slick == "undefined") {
            console.error(
                "Biolife-Framework need to use slick library inside, pls make sure It was loaded before used"
            );
            return;
        }
        ...
        $(this)
            ...
            .each(function () {
                ...
                if (config.vertical === true) {
                    config.prevArrow =
                        '<span class="biolife-icon icon-left-arrow prev"></span>';
                    config.nextArrow =
                        '<span class="biolife-icon icon-arrow-right next"></span>';
                } else {
                    config.prevArrow =
                        '<span class="biolife-icon icon-left-arrow prev"></span>';
                    config.nextArrow =
    ...
    $.fn.biolife_tab = function () {
        $(this).each(function (index, element) {
            ...
            TAB.on("click", ".tab-link", function (e) {
                ...
                if (
                    !$tab_head.hasClass("active") &&
                    typeof tab_id !== undefined
                ) {
    ...
    $.fn.biolife_countdown = function () {
        if (typeof $.fn.countdown == "undefined") {
            console.error(
                "Biolife-Framework need to use countdown library inside, pls make sure It was loaded before used"
            );
            return;
        }
        let $this = $(this),
        ...
        string_format +=
            '<span class="hours"><span class="number">%-H</span><span class="text">Hours</span></span>';
        string_format +=
        ...
        html_result +=
            '<span class="days"><span class="number">00</span><span class="text">days</span></span>';
        html_result +=
            '<span class="hours"><span class="number">00</span><span class="text">Hours</span></span>';
        html_result +=
            '<span class="mins"><span class="number">00</span><span class="text">Mins</span></span>';
        html_result +=
            '<span class="secs"><span class="number">00</span><span class="text">Secs</span></span>';

        $this
            .on("biolife_countdown", function () {
                $this.each(function () {
                    let $element = $(this),
    ...
    $.fn.biolife_sidebar_handle = function () {
        if ($clone_to_mobile_sidebar.length) {
            if ($sidebar.length) {
                ...
                html_sidebar +=
                    '<div id="sidebar" class="sidebar mobile-version">';
                ...
                if ($main_content.length) {
                    ...
                    if ($sidebar_content.length) {
                        $clone_to_mobile_sidebar
                            .clone()
                            .appendTo($sidebar_content);
                    }
                    let $caroulsel_inside =
                        $sidebar_content.find(".biolife-carousel");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Function to update carousel view and indicators
    function updateCarousel() {
        const items = document.querySelectorAll(".carousel-item");
        const totalItems = items.length;

        // Ensure the carousel loops back when reaching the end
    ...
    // Add event listeners to control buttons (Prev/Next)
    prevButton.addEventListener("click", function () {
        currentIndex--; // Decrement the index for previous slide
        updateCarousel(); // Update the carousel position
        resetAutoSlide(); // Reset auto-slide when user manually navigates
    });

    nextButton.addEventListener("click", function () {
        currentIndex++; // Increment the index for next slide
        updateCarousel(); // Update the carousel position
        resetAutoSlide(); // Reset auto-slide when user manually navigates
    });

    // Add event listeners to indicators (clicking on a dot jumps to that slide)
    function addIndicators() {
        ...
        items.forEach((_, index) => {
            ...
            indicator.addEventListener("click", function () {
                currentIndex = index; // Set the current index to the clicked indicator
                updateCarousel(); // Update the carousel position
                resetAutoSlide(); // Reset auto-slide when user manually clicks indicator
            });
            indicators.appendChild(indicator);
        });
    }

    // Start automatic slide change at regular intervals
    ...
    // Reset the auto-slide timer when the user interacts
    function resetAutoSlide() {
        stopAutoSlide(); // Stop the auto-slide when user interacts
        startAutoSlide(); // Restart the auto-slide
    }

    // Initialize the carousel with indicators and the first slide
    addIndicators();
    updateCarousel(); // Initial setup to show the first slide
